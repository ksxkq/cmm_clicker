...
19. MINI 面板策略收敛：MINI 统一为最小可点热点（无内容），点击恢复运行面板；进入 MINI 与拖拽结束均执行左右贴边吸附（带动画），确保单手操作与遮挡最小化。
...
62. MINI 面板状态独立与运行态视觉语义（阶段二-交互稳定）：
   - MINI 卡片宽度调整为 `28dp`（高度保持 `56dp`），以符合“最小可点击贴边热点”目标
   - 新增 MINI 独立位置模型：FULL 与 MINI 分别维护偏移状态，显示模式切换时恢复各自位置，避免“最小化后覆盖普通面板位置”
   - MINI 位置信息按百分比持久化（`KEY_MINI_PANEL_X_RATIO/KEY_MINI_PANEL_Y_RATIO`），按当前屏幕可移动区间恢复，适配不同分辨率/窗口尺寸
   - 运行态 MINI 根据左/右贴边动态切换卡片圆角（贴边侧去圆角），并移除描边以实现无缝贴边视觉
   - 运行态 MINI 增加黄色闪电呼吸动画（`Icons.Rounded.Bolt`），作为持续执行中的统一状态提示
63. RUNNING MINI 交互细节修正（阶段二-交互稳定）：
   - RUNNING MINI 拖动期间强制四角圆角，避免拖动中出现“贴边裁角”导致的视觉跳变
   - RUNNING MINI 恢复 `outline` 描边，保证浅色背景下的可识别性
   - RUNNING 面板最小化进入 MINI 时采用“当前位置 override + 不自动吸边”，保证 x 坐标与缩小前一致；恢复 FULL 仍回到缩小前位置
64. RUNNING MINI 触发即时贴边（阶段二-交互稳定）：
   - 在保留“当前位置 override”作为进入起点的前提下，恢复 RUNNING MINI 的 `enter` 吸边动画，确保触发最小化后立即贴边
   - MINI 吸边与贴边判定统一使用 MINI 目标宽度，而非过渡瞬间的当前测量宽度，避免“二次缩小时判定已贴边而跳过吸边”的误判
65. 主面板窗口命中区域修正（阶段二-交互稳定）：
   - 非过渡锁定态将主面板窗口尺寸策略恢复为 `WRAP_CONTENT`，避免固定壳层的透明区域拦截下层触摸
   - 仅在模式切换锁定阶段维持窗口锁定尺寸，过渡后自动回到 `WRAP_CONTENT`
66. RUNNING 最小化贴边收口（阶段二-交互稳定）：
   - RUNNING 面板最小化进入 MINI 时，吸边改为无位移动画即时贴边，避免与模式切换动画叠加导致“边缘留缝后再贴边”的视觉抖动
   - 拖拽结束吸边与其它入口的吸边动画保持不变
